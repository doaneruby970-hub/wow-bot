...
# --- 导航参数 ---
NAV_ARRIVE_DIST = 0.5
CUSTOM_PATH_FILE = os.path.join(BASE_DIR, 'paths', 'map_12feixingdianxunluA.json')
...
class Navigator:
    ...
    def load_path(self, map_id):
        filename = CUSTOM_PATH_FILE if CUSTOM_PATH_FILE else os.path.join(BASE_DIR, 'paths', f"map_{map_id}.json")
...
def main_controller():
    ...
    loot_combat_start = None  # 捡尸体时被攻击的计时
        ...
        while True:
            ...
            if current_state == "COMBAT":
                ...
                # 2. 距离近，站定攻击！
                    ...
                    elif combat and dist <= 10.0 and time.time() - combat_timer >= 5.0 and t_hp_pct > 0:
                ...
                # 判断怪物是否死亡或丢失（combat=True时不误判）
                if (not has_target or t_hp_pct == 0) and not combat:
                    print("💀 目标倒地或脱战，战斗结束。")
                    pydirectinput.keyUp('w')
            ...
            if current_state == "找尸体":
                if combat:
                    if loot_combat_start is None:
                        loot_combat_start = time.time()
                        print("⚔️ 捡装备中被攻击！持续攻击10秒...")
                    pydirectinput.press(ATTACK_KEY)
                    if time.time() - loot_combat_start >= 10.0:
                        with state.lock:
                            _t_hp = state.target_hp_pct
                        if _t_hp == 0:
                            print("🔄 目标HP为0，按Tab切换目标...")
                            pydirectinput.press('tab')
                        loot_combat_start = None
                        current_state = "COMBAT"
                    continue
                loot_combat_start = None
            ...
            if current_state == "ALIGN_DIRECTION":
                ...
                if dist > ATTACK_DIST:
                    ...
                    while True:
                        ...
                        if dist >= 60.0:
                            pydirectinput.keyUp('w')
                            print(f"⚠️ 目标距离 {dist:.1f} 码，超出60码，放弃追击 -> PATROL")
                            current_state = "PATROL"
                            break
                        time.sleep(0.1)
                    pydirectinput.keyUp('w')
