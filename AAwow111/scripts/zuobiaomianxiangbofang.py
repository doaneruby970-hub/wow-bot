...
PATHS_DIR = "paths"
STOP_KEY = 'page down'
OVERRIDE_PATH_FILE = None  # 供 Master.py 调用时注入路径
...
def player():
    """供 Master.py 调用，使用 OVERRIDE_PATH_FILE"""
    if OVERRIDE_PATH_FILE:
        path_file = os.path.join(PATHS_DIR, OVERRIDE_PATH_FILE)
    else:
        selected = select_path_file()
        if not selected: return
        path_file = os.path.join(PATHS_DIR, selected)
...
def execute_task():
    selected = select_path_file()
    if not selected: return
    with open(os.path.join(PATHS_DIR, selected), 'r') as f:
        _run(json.load(f))
